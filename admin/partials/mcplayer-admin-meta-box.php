<?php
    ...
    */
    ...
    add_filter('posts_join', 'music_search_join' );
    function music_search_join ($join){
        global $pagenow, $wpdb;
        // I want the filter only when performing a search on edit page of Custom Post Type named "music"
        if ( is_admin() && $pagenow=='edit.php' && $_GET['post_type']=='music' && $_GET['s'] != '') {    
            $join .='LEFT JOIN '.$wpdb->postmeta. ' ON '. $wpdb->posts . '.ID = ' . $wpdb->postmeta . '.post_id ';
        }
        return $join;
    }

    add_filter( 'posts_where', 'music_search_where' );
    function music_search_where( $where ){
        global $pagenow, $wpdb;
        // I want the filter only when performing a search on edit page of Custom Post Type named "music"
        if ( is_admin() && $pagenow=='edit.php' && $_GET['post_type']=='music' && $_GET['s'] != '') {
            $where = preg_replace(
